...
const (
	...
	agentKeyFilePath   = "fixtures/agent.key"
	agentCertFilePath  = "fixtures/agent.crt"
)

var _ = Describe("provides an http proxy to a consul cluster", func() {
	var (
	...
	Context("main", func() {
		It("proxies requests to the targeted consul server", func() {
			consulServer := httptest.NewServer(handler)

			command := exec.Command(pathToConsulProxy,
				"--consul-url", consulServer.URL,
				"--port", port,
			)
		...
		It("encrypts traffic to the consul server", func() {
			consulServer := httptest.NewUnstartedServer(handler)
			...
			command := exec.Command(pathToConsulProxy,
				...
				"--cert", agentCertFilePath,
				"--key", agentKeyFilePath,
			)
	...
	Context("failure cases", func() {
		It("returns an error when an unknown flag is provided", func() {
			var err error
			command := exec.Command(pathToConsulProxy, "--some-unknown-flag")
			session, err = gexec.Start(command, GinkgoWriter, GinkgoWriter)
		...
		It("returns an error when a malformed consul-url is provided", func() {
			var err error
			command := exec.Command(pathToConsulProxy, "--consul-url", "%%%%%")
			session, err = gexec.Start(command, GinkgoWriter, GinkgoWriter)
			Eventually(session).Should(gexec.Exit())

			Expect(err).NotTo(HaveOccurred())
			Expect(session.ExitCode()).To(Equal(1))
			Expect(session.Err.Contents()).To(ContainSubstring("failed to parse consul-url parse %%%%%: invalid URL escape \"%%%\""))
		})

		It("returns an error when the cert file path does not exist", func() {
			var err error
			command := exec.Command(pathToConsulProxy, "--require-ssl", "--cert", "/some/fake/path")
			session, err = gexec.Start(command, GinkgoWriter, GinkgoWriter)
			Eventually(session).Should(gexec.Exit())
		...
		It("returns an error when the ca cert file path does not exist", func() {
			...
			command := exec.Command(pathToConsulProxy, args...)
			session, err = gexec.Start(command, GinkgoWriter, GinkgoWriter)
			Eventually(session).Should(gexec.Exit())
		...
		It("returns an error when the proxy fails to start", func() {
			var err error
			command := exec.Command(pathToConsulProxy, "--port", "-1")
			session, err = gexec.Start(command, GinkgoWriter, GinkgoWriter)
			Eventually(session).Should(gexec.Exit())
